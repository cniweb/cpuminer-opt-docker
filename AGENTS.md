...
- Caveat: `README.md` carries its version refs in the `## Version` section, and they must match the workflow's regexes exactly (`` `Dockerfile` currently uses cpuminer-opt `X`. `` / `` `build.sh` currently tags/pushes `X`. ``) or that step silently skips them.
...
- 2026-09-24 — Cross-file contradictions fixed in the source files (canonical flag in `copilot-instructions.md`, `CHANGELOG` wording in `create-release.prompt.md`, version refs added to `README.md` `## Version`, `.serena/` added to `.dockerignore`); the corresponding open-contradictions entry was removed.
